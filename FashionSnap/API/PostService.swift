...
//            }
    ...
    static func checkIfLiked(post: Post, completion: @escaping(Bool) -> Void ){
        guard let  uid = Auth.auth().currentUser?.uid else { return }
    ...
    static func updateUserFeedAfterFollowings(user: User, didFollow: Bool){
        guard let  uid = Auth.auth().currentUser?.uid else { return }
    ...
    static func fetchFeedPostsForFollowed(completion: @escaping([Post]) -> Void){
        guard let uid = Auth.auth().currentUser?.uid else { return }
        var posts = [Post]()
